...
(function (GameMapUtils) {
    ...
    class MgrsGraticule extends L.Layer {
        ...
        __draw(drawLines = false) {
            ...
            if (L.Browser.canvas && map) {
                const dpr = window.devicePixelRatio || 1;
                const ww = canvas.width / dpr, hh = canvas.height / dpr, originX = grid.options.originX, originY = grid.options.originY, sizeInMeters = grid.options.sizeInMeters, fontColor = this.options.fontColor, labelBackground = this.options.labelBackground;
                ...
                let gridInterval = 1000, gridPrecision = 2;
                if (kmPx < 50) {
                    gridInterval = 10000;
                    gridPrecision = 1;
                }
                const latGap = (-originY) % gridInterval, lngGap = (-originX) % gridInterval, boldInterval = gridInterval * 10;
                let latTicks = [];
                let lngTicks = [];
                if (gridInterval > 0) {
    ...
    GameMapUtils.MgrsGraticule = MgrsGraticule;
    function mgrsGraticule(options) {
