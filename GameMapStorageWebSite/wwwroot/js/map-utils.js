...
GameMapUtils.MapToolBase = L.Layer.extend({
    ...
    options: {
        widthInMeters: 1500,
        heightInMeters: 1500,
        dragMarkerClassName: 'protractor-drag',
    ...
    setPosition: function (pos) {
        this._dragMarker.setPosition(pos);
        this._positionChanged();
    },

    getPosition: function () {
        return this._dragMarker.getPosition();
    },

    setBearing: function (bearing) {
        this._bearing = bearing;
        if (this._svgRootGroup) {
            this._svgRootGroup.setAttribute("transform", "rotate(" + bearing + ",51,51)");
        }
    ...
    onAdd: function (map) {
        ...
        if (!this._dragMarker) {
            ...
            this._svgOverlay = L.svgOverlay(this._svgElement, this._toSvgBounds()).addTo(map);
            this._dragMarker =
                L.marker(
                    this._latlng,
                    {
...
GameMapUtils.CoordinateScale = GameMapUtils.MapToolBase.extend({
    options: {
        widthInMeters: 1500,
        heightInMeters: 1500,
        dragMarkerClassName: 'coordinatescale-drag',
